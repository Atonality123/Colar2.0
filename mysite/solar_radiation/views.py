...
from .forms import MyForm, member, share_button
...
def game(request):
    ...
    if request.method == 'POST':
        form = share_button(request.POST)
        return redirect('/colar/game/play')
    else:
        form = share_button()
    context = {'form': form}
    return HttpResponse(template.render(context, request))
...
def user(request):
    ...
    current_datetime = datetime.now()
    api_key = "99782f6213e47bab2358fba8dd9a0fc4"
...
def solar_energy(request):
    template = loader.get_template('solar_energy.html')
    context = {}
    return HttpResponse(template.render(context, request))

def solar_radiation(request):
    template = loader.get_template('solar_radiation.html')
    context = {}
    return HttpResponse(template.render(context, request))

def energy_consumption(request):
    template = loader.get_template('energy_consumption.html')
    context = {}
    return HttpResponse(template.render(context, request))